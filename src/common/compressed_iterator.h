...
template <typename T>
class NewCompressedIterator {
 public:
  // Type definitions for thrust
  typedef NewCompressedIterator<T> self_type;  // NOLINT
  typedef ptrdiff_t difference_type;        // NOLINT
  typedef T value_type;                     // NOLINT
  typedef value_type *pointer;              // NOLINT
  typedef value_type& reference;             // NOLINT
 ...
 public:
  NewCompressedIterator() = default;
  NewCompressedIterator(CompressedByteT *buffer, size_t num_symbols)
      : buffer_(buffer) {
    symbol_bits_ = SmallestWordSize(num_symbols);
  }
  static size_t CalculateBufferSize(size_t num_elements, size_t num_symbols) {
    return SmallestWordSize(num_symbols)*num_elements;
  }

  XGBOOST_DEVICE void Write(size_t idx, T x) {
    if (symbol_bits_ == 8) {
      buffer_[idx] = x;
  ...
  XGBOOST_DEVICE value_type operator[](size_t idx) const {
    if (symbol_bits_ == 8) {
      ...
      return reinterpret_cast<uint16_t *>(buffer_)[idx];
    } else {
      return reinterpret_cast<uint32_t *>(buffer_)[idx];
    }
  }
};
}  // namespace common
