  ...
  struct Evaluator {
   ...
   private:
    ...
    Span<T> UnpackHDVOnDevice(HostDeviceVector<T>* _vec) const {
      auto span = _vec->DeviceSpan(device_);
      return span;
    }
    template <typename T>
    Span<T const> UnpackHDVOnDevice(const HostDeviceVector<T>* _vec) const {
      auto span = _vec->ConstDeviceSpan(device_);
      return span;
    }
    // CPU UnpackHDV
    template <typename T>
    Span<T> UnpackHDV(HostDeviceVector<T>* _vec) const {
      return Span<T>{_vec->HostPointer(), static_cast<typename Span<T>::index_type>(_vec->Size())};
    }
    template <typename T>
    ...
    void LaunchCUDA(Functor _func, HDV*... _vectors) const {
      ...
      const int kGrids = static_cast<int>(DivRoundUp(*(range_.end()), kBlockThreads));
      if (kGrids == 0) {
        return;
      }
