...
TEST(EllpackPage, BuildGidxDense) {
  ...
  std::vector<uint32_t> solution = {
      0,  3,  8,  9,  14, 17, 20, 21, 0,  4,  7,  10, 14, 16, 19, 22, 1,  3,  7,  11, 14, 15,
      19, 21, 2,  3,  7,  9,  13, 16, 20, 22, 2,  3,  6,  9,  12, 16, 20, 21, 1,  5,  6,  10,
      13, 16, 20, 21, 2,  5,  8,  9,  13, 17, 19, 22, 2,  4,  6,  10, 14, 17, 19, 21, 2,  5,
      7,  9,  13, 16, 19, 22, 0,  3,  8,  10, 12, 16, 19, 22, 1,  3,  7,  10, 13, 16, 19, 21,
      1,  3,  8,  10, 13, 17, 20, 22, 2,  4,  6,  9,  14, 15, 19, 22, 1,  4,  6,  9,  13, 16,
      19, 21, 2,  4,  8,  10, 14, 15, 19, 22, 1,  4,  7,  10, 14, 16, 19, 21,
  };
...
TEST(EllpackPage, BuildGidxSparse) {
  ...
  std::vector<uint32_t> solution = {15, 24, 24, 0,  24, 24, 24, 24, 24, 24, 24, 24, 20, 24, 24, 24,
...
TEST(EllpackPage, FromCategoricalMissing) {
  ...
  p_fmat->Info().feature_types.ConstDeviceSpan(ctx.Device());
...
class CompressedDense : public ::testing::TestWithParam<std::size_t> {
 ...
 public:
  ...
  void CheckFromAdapter(std::size_t null_column) {
    ...
    data.ConstDeviceSpan(data.Device());  // Pull to device
 ...
 protected:
  ...
  void TestNumNonMissing(float sparsity) const {
    ...
    for (auto const& page : p_fmat->GetBatches<EllpackPage>(
      ...
      auto ellpack_nnz = page.Impl()->NumNonMissing(
          &ctx, p_fmat->Info().feature_types.ConstDeviceSpan(p_fmat->Info().feature_types.Device()));
